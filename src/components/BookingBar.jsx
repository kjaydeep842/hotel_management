...
const BookingBar = ({ hotelName, price, onUpdateSearch }) => {
    ...
    // Toggle full edit mode for mobile
    const [isEditing, setIsEditing] = useState(false);

    // Helper to format Date like "02 Apr 2026"
    ...
    return (
        <>
            {/* Collapsed Mobile View (Hidden on LG or when editing) */}
            <div className={`lg:hidden ${isEditing ? 'hidden' : 'flex'} bg-[#f8f9fa] border-b border-gray-200 p-3 sm:p-4 justify-between items-center sticky top-[60px] z-40 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05)] transition-all duration-300`}>
                <div className="flex flex-col flex-1 pr-4 min-w-0">
                    <span className="text-[#4a72a8] text-sm sm:text-base font-medium truncate tracking-wide">{hotelName || 'Taj Bekal Resort & Spa, Kerala'}</span>
                    <span className="text-gray-500 text-[10px] sm:text-xs mt-0.5 truncate tracking-wide">
                        {formatDate(startDate)} — {formatDate(endDate)} | {adults + children} Guest{adults + children > 1 && 's'}, {rooms} Room{rooms > 1 && 's'}
                    </span>
                </div>
                <button
                    onClick={() => setIsEditing(true)}
                    className="bg-[#b4974f] hover:bg-[#a38847] text-white text-[11px] sm:text-xs font-bold py-2 sm:py-2.5 px-4 sm:px-6 rounded-sm uppercase transition-colors shrink-0 shadow-sm"
                >
                    Edit
                </button>
            </div>
            ...
            {/* Expanded Full View */}
            <div className={`${!isEditing ? 'hidden lg:block' : 'block'} bg-gradient-to-r from-orange-500 to-red-500 dark:from-orange-600 dark:to-red-600 p-3 lg:p-4 shadow-lg sticky top-[60px] lg:top-20 z-40 transform transition-all duration-300`}>
                {isEditing && (
                    <div className="lg:hidden flex justify-between items-center mb-3">
                        <span className="text-white font-bold text-sm uppercase tracking-wider">Update Booking</span>
                        <button onClick={() => setIsEditing(false)} className="text-white/80 hover:text-white bg-black/10 hover:bg-black/20 p-1.5 rounded-full transition-colors flex items-center justify-center">
                            <Minus className="h-4 w-4" />
                        </button>
                    </div>
                )}

                <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-end gap-3 lg:gap-4">

                    {/* Hotel Name / Location */}
                    <div className="flex-1 w-full">
                        <label className="text-white text-[10px] lg:text-xs font-bold uppercase mb-1 block">Area, Landmark or Property Name</label>
                        <div className="relative">
                            <Home className="absolute left-2.5 lg:left-3 top-2.5 lg:top-3 h-4 w-4 lg:h-5 lg:w-5 text-gray-400" />
                            <input
                                type="text"
                                value={hotelName}
                                readOnly
                                className="w-full pl-8 lg:pl-10 pr-2 lg:pr-4 py-2 lg:py-2.5 rounded-md bg-white text-gray-900 border-none focus:ring-2 focus:ring-white/50 font-bold truncate text-sm lg:text-base"
                            />
                        </div>
                    </div>

                    {/* Check-in and Check-out Row for Mobile */}
                    <div className="flex flex-row w-full lg:w-auto gap-3">
                        {/* Check-in Date */}
                        <div className="w-1/2 lg:w-48">
                            <label className="text-white text-[10px] lg:text-xs font-bold uppercase mb-1 block">Check-in</label>
                            <div className="relative">
                                <Calendar className="absolute left-2.5 lg:left-3 top-2.5 lg:top-3 h-4 w-4 lg:h-5 lg:w-5 text-gray-400 z-10" />
                                <DatePicker
                                    selected={startDate}
                                    ...
                                    className="w-full pl-8 lg:pl-10 pr-2 lg:pr-4 py-2 lg:py-2.5 rounded-md bg-white text-gray-900 border-none focus:ring-2 focus:ring-white/50 font-bold text-sm lg:text-base cursor-pointer"
                        ...
                        {/* Check-out Date */}
                        <div className="w-1/2 lg:w-48">
                            <label className="text-white text-[10px] lg:text-xs font-bold uppercase mb-1 block">Check-out</label>
                            <div className="relative">
                                <Calendar className="absolute left-2.5 lg:left-3 top-2.5 lg:top-3 h-4 w-4 lg:h-5 lg:w-5 text-gray-400 z-10" />
                                <DatePicker
                                    selected={endDate}
                    ...
                    <div className="w-full lg:w-72 relative" ref={dropdownRef}>
                        <label className="text-white text-[10px] lg:text-xs font-bold uppercase mb-1 block">Guest & Rooms</label>
                        <div className="relative">
                            <Users className="absolute left-2.5 lg:left-3 top-2.5 lg:top-3 h-4 w-4 lg:h-5 lg:w-5 text-gray-400 z-10" />
                            <button
                                onClick={() => setIsOpen(!isOpen)}
                                className="w-full pl-8 lg:pl-10 pr-2 lg:pr-4 py-2 lg:py-2.5 rounded-md bg-white text-gray-900 border-none focus:ring-2 focus:ring-white/50 font-bold flex items-center justify-between text-sm lg:text-base"
                            >
                    ...
                    <div className="w-full lg:w-auto">
                        <button
                            onClick={() => {
                                setIsEditing(false);
                                if (onUpdateSearch) onUpdateSearch({ startDate, endDate, rooms, adults, children });
                            }}
                            className="w-full bg-white hover:bg-gray-100 text-blue-600 font-bold py-2 lg:py-2.5 px-6 rounded-md shadow-md transition-colors uppercase tracking-wide text-sm lg:text-base"
                        >
                            Update Search
                        </button>
                    </div>
